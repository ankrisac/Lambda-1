...
char VL_ModuleSrc_match_escape_seq(char in){
    switch(in){
        case 'n': return '\n';
        case 't': return '\t';
        case 'r': return '\r';
...
bool VL_ModuleSrc_parse_Number(VL_ModuleSrc* self, VL_ParseState* state){
    ...
    if(!VL_match_digit(VL_ModuleSrc_peek(self, state))){
        VL_ModuleSrc_push_err_str(self, &begin, state,
    ...
    VL_Int Int = 0;
    for(char chr; VL_match_digit(chr = VL_ModuleSrc_peek(self, state)); VL_ModuleSrc_next(state, chr)){
        Int = Int * 10 + (VL_Int)(chr - '0');
    }
    
    if(!VL_ModuleSrc_match_chr(self, state, '.')){
...
bool VL_ModuleSrc_parse_String(VL_ModuleSrc* self, VL_ParseState* state){
    ...
    while(true){
        chr = VL_ModuleSrc_peek(self, state);
        if(!escaped){
            if(chr != '"' && chr != '\0'){
                if(chr != '\\'){
            ...
            VL_Str_append_char(string, VL_ModuleSrc_match_escape_seq(chr));
            escaped = false;
        }   
...
bool VL_ModuleSrc_parse_Label(VL_ModuleSrc* self, VL_ParseState* state){
    ...
    if(!VL_ModuleSrc_parse_token_symbol_label(VL_ModuleSrc_peek(self, state))){
        VL_ModuleSrc_push_err_str(self, &begin, state,
    ...
    for(char chr; VL_ModuleSrc_parse_token_symbol(chr = VL_ModuleSrc_peek(self, state)); VL_ModuleSrc_next(state, chr)){
        VL_Str_append_char(string, chr);
    }
...
bool VL_ModuleSrc_parse_Operator(VL_ModuleSrc* self, VL_ParseState* state){
    ...
    if(VL_ModuleSrc_parse_token_symbol_operator(chr = VL_ModuleSrc_peek(self, state))){
        VL_Str* string = VL_Str_new(1);

        VL_Str_append_char(string, chr);
        VL_ModuleSrc_next(state, chr);

        for(; VL_ModuleSrc_parse_token_symbol(chr = VL_ModuleSrc_peek(self, state)); VL_ModuleSrc_next(state, chr)){
            VL_Str_append_char(string, chr);
        }
...
bool VL_ModuleSrc_parse_FAtom(VL_ModuleSrc* self, VL_ParseState* state){
    VL_ParseState begin = *state;
    switch(VL_ModuleSrc_peek(self, state)){
        case '"':
            return VL_ModuleSrc_parse_String(self, state);
        case '0' ... '9':
            return VL_ModuleSrc_parse_Number(self, state);
        case '(':
            VL_ModuleSrc_next(state, '(');
            
            if(VL_ModuleSrc_parse_IExpr(self, state)){
                if(!VL_ModuleSrc_match_chr(self, state, ')')){
                    VL_ModuleSrc_push_err_str(self, &begin, state,
        ...
        case '[':
            return VL_ModuleSrc_parse_LExpr(self, state);
        case '{':
            return VL_ModuleSrc_parse_BExpr(self, state);
        case ']':
        case '}':  
        case ';':
        case ',':
        ...
        default:
            return VL_ModuleSrc_parse_Label(self, state);
    }
    return false;
}
bool VL_ModuleSrc_parse_FExpr(VL_ModuleSrc* self, VL_ParseState* state){
    ...
    if(!VL_ModuleSrc_parse_FAtom(self, state)){
        return false;
    }            
    ...
    size_t i = 0;
    while(true){
        if(!VL_ModuleSrc_parse_SpaceSep(self, &temp)){ break; }
        if(!VL_ModuleSrc_parse_FAtom(self, &temp)){ break; }
...
bool VL_ModuleSrc_parse_LExpr(VL_ModuleSrc* self, VL_ParseState* state){
    ...
    VL_ModuleSrc_parse_Space(self, state);
    VL_ParseState temp = *state;

    if(VL_ModuleSrc_parse_IExpr(self, &temp)){
        VL_Expr_append_Object(expr, temp.val, state->p, temp.p);
        *state = temp;
        ...
        while(true){
            ...
            temp = *state;
            if(!VL_ModuleSrc_match_chr(self, &temp, ',')){
                break;
            }
                
            VL_ModuleSrc_parse_Space(self, &temp);
...
bool VL_ModuleSrc_parse_BExpr(VL_ModuleSrc* self, VL_ParseState* state){
    ...
    while(true){
        ...
        *state = temp;
        ...
        if(!VL_ModuleSrc_parse_IExpr(self, &temp)){ break; }
...
bool VL_ModuleSrc_parse_IExpr(VL_ModuleSrc* self, VL_ParseState* state){
    ...
    if(!VL_ModuleSrc_parse_FExpr(self, state)){
        VL_Expr_delete(expr);
    ...
    while(true){
        ...
        if(!VL_ModuleSrc_parse_Operator(self, &temp)){ 
            VL_ModuleSrc_pop_errors(self, p_err, self->error_stack->len);
            break;
        }
        ...
        *state = temp;
        ...
        if(!VL_ModuleSrc_parse_SpaceSep(self, &temp)){
            VL_Expr_delete(expr);
            VL_ModuleSrc_push_err_str(self, &begin, state,
        ...
        if(!VL_ModuleSrc_parse_FExpr(self, &temp)){ 
            VL_Expr_delete(expr);
            VL_ModuleSrc_push_err_str(self, &begin, state,
...
void VL_ModuleSrc_parse_NExpr(VL_ModuleSrc* self, VL_ParseState* state){
    ...
    VL_ModuleSrc_parse_Space(self, state);
    VL_ModuleSrc_parse_IExpr(self, state);
    if(!state->ok){ return; }
    VL_ModuleSrc_parse_Space(self, state);
...
VL_ParseState VL_ModuleSrc_parse_Lisp(VL_ModuleSrc* self, VL_ParseState in){
    VL_ModuleSrc_parse_NExpr(self, &in);
    return in;
}
