...
void VL_Core_trace(VL_Core* self, const VL_ExprAtom* atom){
...
void VL_Core_error(VL_Core* self, const VL_ExprAtom* source){
    ...
    VL_Core_trace(self, source);
}

void VL_Core_stack_push(VL_Core* self, VL_Object* obj){
    VL_Tuple_append(self->stack, obj);
}
void VL_Core_stack_push_copy(VL_Core* self, const VL_Object* obj){
...
void VL_Core_stack_push_error(VL_Core* self, VL_Error error){
    VL_Tuple_append(self->stack, 
...
BINARY_FUNC(div, VL_KEYWORD_DIV, 
    ...
    CTYPE(INT,
        if(rhs->data.v_int != 0){
            ...
            self->stack->len--;
            return;
        }else{  FUNC_KEYWORD_ERR(2, VL_KEYWORD_DIV, 
                    printf(" ");
                    VL_Object_perror(lhs);
                    printf("/");
                    VL_Object_perror(rhs);
...
bool VL_Core_eval_symbol(VL_Core* self, VL_SymMap* env, const VL_Symbol* sym){
    ...
    VL_Core_stack_push_error(self, VL_ERROR_SYMBOL_UNDEFINED);
    return false;
}

#define EVAL(EXPR_LOOP)                                             \
    switch(obj->type){                                              \
        CTYPE(EXPR, const VL_Expr* expr = obj->data.expr; EXPR_LOOP)\
        CTYPE(SYMBOL,                                               \
...
void VL_Core_eval_ast(VL_Core* self, VL_SymMap* env, const VL_Object* obj){
    EVAL(
...
void VL_Core_eval_obj(VL_Core* self, VL_SymMap* env, VL_Object* obj){
    ...
    #define SFORM_FUNCTION(KEYWORD, IS_MACRO)                           \
        SFORM_CASE(KEYWORD, 2,                                          \
            ...
            }else{  VL_Keyword_perror(VL_KEYWORD_GET_ENUM(KEYWORD));    \
    ...
    #define KEYWORD_ERROR(KEYWORD, N, MSG)                  \
        VL_Keyword_perror(VL_KEYWORD_GET_ENUM(KEYWORD));    \
        printf(MSG);                                        \
        VL_Core_error(self, VL_Expr_get(expr, N));          \
        VL_Core_stack_push_error(self, VL_ERROR_TYPE_ERROR);

    #define EVAL_ERROR(N, I, MSG)                               \
        size_t args = VL_Core_num_args(self, fn_ptr);           \
        VL_Object_perror(VL_Core_stack_get(self, fn_ptr + I));  \
        printf(MSG);                                            \
        VL_Core_error(self, VL_Expr_get(expr, I));              \
        VL_Core_stack_dropn(self, args + 1);                    \
        VL_Core_stack_push_error(self, VL_ERROR_ARG_MISMATCH);       

    #define SFORM_TYPECHECK(KEYWORD, N, OBJECT_TYPE, TYPE_ENUM, EXPR)   \
        if(OBJECT_TYPE == VL_TYPE_GET_ENUM(TYPE_ENUM)){ EXPR }          \
        else{   VL_Keyword_perror(VL_KEYWORD_GET_ENUM(KEYWORD));        \
                printf(" requires argument to be ");                    \
                VL_Type_perror(VL_TYPE_GET_ENUM(TYPE_ENUM));            \
                printf("\n");                                           \
                VL_Core_trace(self, VL_Expr_get(expr, N));              \
                VL_Core_stack_push_error(self, VL_ERROR_TYPE_ERROR); }

    do{
        VL_Core_macro_expand(self, env, ast);
        tail_call = special_form = false;
        ...
        if(special_head->type == VL_TYPE_KEYWORD){
            ...
            switch(special_head->data.keyword){
                SFORM_CASE(DO, 1, 
                    tail_call = (ast = VL_Core_eval_do(self, env, VL_Expr_get(expr, 1)->val)) != NULL;)
                SFORM_CASE(MACROEXPAND, 2, 
                    ...
                    SFORM_TYPECHECK(MACROEXPAND, 1, n_expand->type, INT, 
                        SFORM_TYPECHECK(MACROEXPAND, 2, macro_expr.type, EXPR,
                            ...
                            VL_Core_stack_push(self, &macro_expr);)))
                SFORM_CASE(QUOTE, 1, VL_Core_stack_push_copy(self, VL_Expr_get(expr, 1)->val);)
                SFORM_CASE(QUASIQUOTE, 1,
                ...
                SFORM_CASE(SET, 2,
                    ...
                    SFORM_TYPECHECK(SET, 1, label->type, SYMBOL,
                        ...
                        VL_Object val; VL_Core_stack_pop_to(self, &val);
                ...
                CKEYWORD(IF,
                    ...
                    switch(expr->len){
                        ...
                        CDEFAULT(KEYWORD_ERROR(IF, 0, " expected only 2-3 arguments!\n"))
                    })
                SFORM_CASE(WHILE, 2,
                    bool loop = true;
